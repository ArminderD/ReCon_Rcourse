...
Below, you will see the data dictionary for utri.csv. The dataset is pretty clean, with most variables in numeric format. We may need to recode some variables as we go along. 
...
``` {r}
x <- table(utri$diabetes, utri$ARI)
```
...
*SHOW MANUAL CALCULATION USING R - not with package!*

```{r }
#### add manual calculations here
...
Before we go any further, there are a couple of variables here that we should examine a bit more closely. Two that had multiple levels were season and location. We should see how best to include these in any multivariable model. Here we are going to use the logistic command, which provides odds ratios.

**Task 6: (5 minutes)** In the R script type the following commands:
```{r, eval=FALSE}
table(utri$season, utri$ARI) 
glm(utri$ARI~ utri$season, family = binomial(link = 'logit')) 
table(utri$location, utri$ARI)
glm(utri$ARI~ utri$location, family = binomial(link = 'logit')) 
```
...
**What do you think about the odds ratios for these variables? What should we do with these when we proceed to including multiple variables in the model?**
    ...
    The odds ratios are not consistent at different levels of these variables. In particular, winter (season 2) had a markedly stronger association with ARI than the reference category, which was autumn (season 1). We should include this variable as indicator variables in future models. For location, the only level that was significantly associated with ARI compared with the baseline of capital/major city was level 3, which was for a rural area. Remote areas had very few study subjects. Here we should create one variable for rural/remote compared to urban and see if that is worth including in future models.


**Task 7: (5 minutes)** In the R script, create a new variable called ‘rural’:
```{r}
	if(utri$location == 3 || utri$location==4){
    utri$rural <- 1	  
      } else utri$rural <- 0
	utri$rural <- factor(utri$rural, levels = c(0,1), labels = c("No", "Yes"))
	table(utri$ARI,utri$rural)
#	      No  Yes
#  0 5536    0
#  1 1374    0
  
	### @@@@@@GLM MODEL NOT WORKING @@@@@@@@ 
	#glm(utri$ARI~ utri$rural, family = binomial(link = 'logit')) 
```
...
library(margins)
summary(margins(fm3))
#to obtain predicted values of ARI from model fm3
pred <- predict(fm3)
#To calculate probability of ARI =1
prob <- exp(pred)/(1+exp(pred))
#To generate interaction plots
interaction.plot(age_cat, sex, prob)
```
...
hoslem.test(ARI, fitted(fm4), g =10)
## c. to plot ROC curve
prob=predict(fm4,type=c("response"))
fm4$prob=prob
library(pROC)
g <- roc(ARI ~ prob, data = utri)
plot(g)  
# d. to calculate AUC
auc(g)
```
